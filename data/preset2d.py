...
class Preset2dHelper:
    def __init__(self, ds: PresetDataset):
        ...
        try:
            cat_params_class_samples_count = ds.cat_params_class_samples_count
        except FileNotFoundError as e:
            cat_params_class_samples_count = None
            warnings.warn(f"cat_params_class_samples_count can't be read by the dataset and will be set to zero. "
                          f"Ignore this warning only during dataset re-generation. Original exception: {e}")
        if cat_params_class_samples_count is not None:
            for vst_idx, class_samples_count in cat_params_class_samples_count.items():
