...
import logs.logger
import logs.metrics
from model.processminibatch import process_minibatch
from logs.metrics import SimpleMetric, EpochMetric, VectorMetric, LatentMetric, LatentCorrMetric
import data.dataset
import data.build
import utils.profiling
from utils.hparams import LinearDynamicParam, CyclicalParam
...
def train_model(model_config: config.ModelConfig, train_config: config.TrainConfig):
    ...
    # FIXME PyCharm issue: remote debugguer fails to attach after the next line has been executed
    #   Remote machine error: "Error loading: ~/.pycharm_helpers/pydev/pydevd_attach_to_process/attach_linux_amd64.so"
    #   Maybe caused by Comet.ml multiprocessing usage...
    #   Temp workaround: launch any command in PyCharm's Console, then debug keeps working
    ...
    scalars = {  # Audio reconstruction negative log prob + monitoring metrics comparable across all models
        ...
        # extra regularization, e.g. attribute-based regularization, contrastive, etc.
        'Latent/ExtraBpLoss/Train': EpochMetric(), 'Latent/ExtraBpLoss/Valid': EpochMetric(),
        ...
        'Sched/VAE/beta': CyclicalParam(
            train_config.beta_cycle, train_config.beta_cycle_duration, current_epoch=train_config.start_epoch),
    ...
    for epoch in range(train_config.start_epoch, train_config.n_epochs):
        ...
        with optional_profiler.get_prof(epoch) as prof:  # TODO use comet context if available
            ...
            for i in range(len(dataloader['train'])):
                ...
                x_in, v_in, uid, notes, labels, attributes = [m.to(device) for m in minibatch]
                process_minibatch(  # we don't need to store the outputs - main logs are done inside this function
                    ae_model, ae_model_parallel, device, x_in, v_in, uid, notes, labels, attributes,
        ...
        if logger.should_validate:  # don't always compute validation (very long w/ transformer AR decoders)
            with torch.no_grad():
                ...
                for i, minibatch in enumerate(dataloader['validation']):
                    x_in, v_in, uid, notes,  labels, attributes = [m.to(device) for m in minibatch]
                    ae_out_audio, ae_out_preset = process_minibatch(
                        ae_model, ae_model_parallel, device, x_in, v_in, uid, notes,  labels, attributes,
                        epoch, scalars, super_metrics
                    )
                    # Validation plots
                    if logger.should_plot:
                        if not pretrain_audio and ae_out_preset is not None:
                        ...
                        if ae_out_audio is not None:
                            if i == i_to_plot:  # random mini-batch plot (validation dataset is not randomized)
                                ...
                                logger.plot_decoder_interpolation(
                                    ae_model, ae_out_audio.z_sampled,
        ...
        if logger.should_plot or early_stop:
            ...
            # This one is very long, but.... we do not plot often
